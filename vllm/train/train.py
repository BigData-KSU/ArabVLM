...
def preprocess_llama_3(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            cur_len += round_len + 1  # starting from index 0, then cur_len will not cover eos token
...
def train():
    ...
    if training_args.lora_enable:
        ...
        ################# LOad model.................
        print('Loading LoRA weights from orignal model')
        from peft import PeftModel
        print(model)
        model = PeftModel.from_pretrained(model, './checkpoints/videollava-8b-lora-llama3-llava-Mix_shunk123456789-PMCFinal_Epoch1')
        print(f"Merging orignal PMC and MIX weights")
        model = model.merge_and_unload()        #### Important:  remove comment when using New LORA, I shoud merge first.
        print('Merged................................Go, ahead.............................')
        #########################################################################################################
        ...
        ##### Important: Remove this one intrcutiion tuning on orignal dataset......
        # and add when finetuing on dowanstream tasks
    ...
    if model_args.version == "v0":
        ...
        tokenizer.pad_token = "<|reserved_special_token_0|>" if not "Llama-3" in model_args.model_name_or_path else tokenizer.eos_token
    ...
    # =============================================================================================================
        ...
        if model_args.tune_mm_mlp_adapter:
            print('Tune MLP adaptor')
            print(model_args.tune_mm_mlp_adapter)
            #exit(0)
